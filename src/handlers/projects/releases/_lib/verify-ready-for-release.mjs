...
const verifyReadyForRelease = ({ mainBranch, originRemote, packageSpec, projectPath, releaseBranch, reporter }) => {
  ...
  if (currentBranch === releaseBranch) reporter.push(`  already on release branch ${releaseBranch}.`)
    ...
    throw createError.BadRequest(`Release branch can only be cut from main branch '${mainBranch}'; current branch: '${currentBranch}'.`)
  ...
  if (updateResult !== 0)
    throw createError.InternalServerError(`Could not update ${originRemote}/${mainBranch}: ${updateResult.stderr}`)

  const orginHead = shell.exec(`git rev-parse ${originRemote}/${mainBranch}`)
  if (originHead !== 0)
    throw createError.InternalServerError(`Could not determnie version for ${originRemote}/${mainBranch}: ${orginHead.stderr}`)
  const localHead = shell.exec(`git rev-parse ${mainBranch}`)
  if (localHead !== 0)
    throw createError.InternalServerError(`Could not determnie version for ${mainBranch}: ${localHead.stderr}`)

  if (originHead.stdout !== localHead.stdout)
    throw createError.BadRequest(`Local and ${originRemote} '${mainBranch} are not in sync. Try:\n\ngit fetch ${originRemote} ${mainBranch} \\\n  && git merge ${originRemote}/${mainBranch}\\\n  && git push ${originRemote} ${mainBranch}`)
  
  reporter?.push('Checking working directory is clean...')
  const cleanResult = shell.exec(`cd '${projectPath}' && git status --porcelain`)
  if (cleanResult.code !== 0)
    throw createError.InternalServerError(`Could not execute 'git status' in dir '${projectPath}'.`)
  else if (cleanResult.stdout.length > 0)
    throw createError.BadRequest(`git repo at '${projectPath}' is not clean.`)
