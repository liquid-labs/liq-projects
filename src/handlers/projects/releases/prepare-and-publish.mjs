...
const func = ({ app, model, reporter }) =>  async(req, res) => {
  ...
  const { githubOrg, packageSpec, projectFQN, projectPath } = pkgData
  ...
  const currentBranch = 
    verifyReadyForRelease({ mainBranch, originRemote, packageSpec, projectPath, originRemote, releaseBranch, reporter })

  if (currentBranch === mainBranch) {
    const checkoutResult = shell.exec(`cd '${projectPath}' && git checkout --quiet -b '${releaseBranch}'`)
    if (checkoutResult.code !== 0)
      throw createError.InternalServerError(`Failed to checkout release branch '${releaseBranch}': ${checkoutResult.stderr}`)
  }
  ...
  if (versionResult.code !== 0)
    throw createError.InternalServerError(`'npm version ${nextVer}' failed; address or update manually.`)
  const pushTagsResult = shell.exec(`cd '${projectPath}' && git push --tags`)
  if (pushTagsResult.code !== 0) 
    throw createError.InternalServerError(`Failed to push version release tag v${nextVer}; address or push manually.`)
  ...
  if (deleteBranchResult.code !== 0) 
    throw createError.InternalServerError(`Could not delete release branch '${releaseBranch}'; manually delete.`)

  const pushResult = shell.exec(`cd '${projectPath}' && git push ${originRemote} ${mainBranch}`)
  if (pushResult.code !== 0)
    throw createError.InternalServerError(`Failed to push merged '${mainBranch}' to remote '${originRemote}'; push manually.`)
