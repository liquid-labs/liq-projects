...
describe('getFiles', () => {
	test('recursively finds all files', async () => {
		...
		expect(results)
			.toEqual([ 'a.txt', 'b/b1.txt', 'b/b2.plain', 'b/d/d1.txt', 'c/c1.txt' ].map((f) => dataRoot + '/' + f))
	})
	...
	test("'depth: 1' limits search to root directory", async () => {
		...
		expect(results)
			.toEqual([ 'a.txt' ].map((f) => dataRoot + '/' + f))
	})
	...
	test("'depth: 2' limits search to root directory and direct child files", async () => {
		...
		expect(results)
			.toEqual([ 'a.txt', 'b/b1.txt', 'b/b2.plain', 'c/c1.txt' ].map((f) => dataRoot + '/' + f))
	})
	...
	test("'reName: (?:.txt) filters out non-'.txt' files", async () => {
		...
		expect(results).toHaveLength(4)
		expect(results)
			.toEqual([ 'a.txt', 'b/b1.txt', 'b/d/d1.txt', 'c/c1.txt' ].map((f) => dataRoot + '/' + f))
	})
})